...
int main() {
    ...
    App::Renderer renderer = App::Renderer(settings);
    renderer.setup();

    // in opt, result is const, not edited.
    App::Optimizer optimizer = App::Optimizer(lset&, result&, settings.optSettings.);
    // I would like to change the phy
    App::Solver solver = App::Solver(settings.solverSettings, result&, lset&);
    App::Exporter exporter = App::Exporter(settings.exporterSettings);
